...
function apply(ctx) {
	...
	*/
	const projectionsFor = (id, coldHeaders) => {
		let values;
		const live = ctx.sessions.get(id);
		if (live !== void 0) values = ctx.sessionProjections.snapshot(live).values;
		else {
			const header = coldHeaders.get(id);
			const cache = ctx.get("sessionProjectionCache");
			if (header !== void 0 && cache !== void 0) values = cache.cachedSnapshot(header)?.values;
		}
		const out = {};
	...
	*/
	const tabFor = async (sessionId) => {
		...
		const coldHeaders = /* @__PURE__ */ new Map();
		try {
			const persistence = ctx.get("sessionPersistence");
			if (persistence !== void 0) for (const header of await persistence.list()) coldHeaders.set(asString(header.id), header);
		} catch {}
		const merged = [];
		const seen = /* @__PURE__ */ new Set();
		for (let index = 0; index < catalog.length; index++) {
